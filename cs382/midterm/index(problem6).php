    ...
    <div class='container-fluid'>
        ...
        <h3>Problem 6: Compute the number of containers needed to complete the order</h4>
            <form method="post">
                ...
                <p><input type='submit' name='submit6' value="Estimate Transportation Charges" class='btn btn-primary'>
                    <input type='reset' class='btn btn-default'>
                </p>
                <?php
                if (isset($_POST["submit6"])) {
                    ...
                    $error = false;
                    if ($cartLength < 20 || $cartLength > 40) {
                        echo "<p class='alert alert-danger'>Please enter the length between 20 and 40</p>";
                        $error = true;
                    }
                    if ($cartHeight < 10 || $cartHeight > 15) {
                        echo "<p class='alert alert-danger'>Please enter the height between 10 and 15</p>";
                        $error = true;
                    }
                    if ($distance < 0) {
                        echo "<p class='alert alert-danger'>Please enter a positive distance</p>";
                        $error = true;
                    }
                    if (!$error) {
                        $volume = 8 * $cartHeight * $cartLength;
                        $containers = ceil(100000 / $volume);
                        $cost = $containers * 30 * $distance;
                        echo "<p class='alert alert-success'>Number of containers needed to move the sand: $containers<br>Cost of transportation: $$cost</p>";
                    }
                }
                ?>
            </form>
